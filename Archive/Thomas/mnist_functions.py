...
def train_and_evaluate_best_model(x_train, y_train, x_test, y_test, layer_output_sizes, activation_funcs, lmbda):
    _, input_size = np.shape(x_train)
    _, output_size = np.shape(y_train)
    cost_fnc = cross_entropy                                  # cost function
    cost_der = cross_entropy_der
    ...
    ffnn.train_network(x_train, y_train, batches =100, optimizer=Adam(0.03), epochs =100, printer=False)
